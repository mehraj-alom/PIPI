...
class MedicalRecordSchema(BaseModel):
    ...
    medications:         Optional[list[str]]      = Field(None, description="List of prescribed medications with dosage also check whether this medication exist if not check what mathes the closest to it and return the name of the medication and the dosage if possible, verfify medication with the disease ..")
    ...
    summary:             Optional[str]            = Field(..., description="A brief summary of the medical record, key findings, or important notes for quick reference,it must be of 150+ characters")
